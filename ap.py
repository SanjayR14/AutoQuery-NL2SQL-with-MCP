...
def validate_sql(query: str) -> None:
    """
    Block clearly unsafe SQL operations.

    This is intentionally simple: it looks for forbidden keywords in the
    uppercased query string. If any are found, it raises a ValueError.
    """
    query_upper = query.upper()

    # Simple multi-statement guardrail: disallow multiple ';'
    if query_upper.count(";") > 1:
        raise ValueError("Unsafe SQL detected: multiple statements are not allowed.")

    for word in FORBIDDEN_SQL_KEYWORDS:
        if word in query_upper:
            raise ValueError(f"Unsafe SQL detected: '{word}' operations are not allowed.")


def enforce_limit(query: str, default_limit: int = 100) -> str:
    """
    Ensure a LIMIT clause exists on SELECT-like queries to prevent full table scans.

    If the query already contains 'LIMIT', it is returned unchanged.
    Otherwise, 'LIMIT <default_limit>' is appended.
    """
    ...
    # Only enforce LIMIT on typical read queries
...
async def execute_query(db_name: str, query: str) -> str:
    ...
    try:
        # Apply SQL guardrails before running anything against the database
        try:
            validate_sql(query)
