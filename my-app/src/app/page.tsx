...
function AppLayout() {
  ...
  const handleSelectTable = async (table: string) => {
    ...
    setRowsError(null);
    setRowsLoading(true);
    setSearchText(`${selectedDatabase}.${table}`);
        ...
        <div className="flex-1 rounded-xl border border-slate-800 bg-slate-950/60 p-3 overflow-auto">
          ...
          {selectedDatabase && (
            <div className="mt-2 border-t border-slate-800 pt-2">
              ...
              {tables.length > 0 && (
                <ul className="space-y-1 text-[11px] max-h-64 overflow-auto mt-1">
                  {tables.map((table) => (
                    <li
                      key={table}
                      ...
                      className={`px-2 py-1 rounded-md border cursor-pointer transition ${
                        selectedTable === table
                          ...
                          : "bg-slate-900/70 text-slate-100 border-slate-700/60 hover:bg-slate-800/80"
                      }`}
                    >
                      {table}
                    </li>
          ...
          <div className="mt-4">
            <div className="flex items-baseline justify-between mb-2">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Showing up to 50 rows from this table. Use the chat assistant to ask for counts or more complex
                  analysis.
                </p>
              </div>
            </div>

            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 max-h-[420px] overflow-auto text-xs">
...
function ChatInput({ inProgress, onSend, isVisible = true, onStop, hideStopButton }: ChatInputProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!searchText.trim() || inProgress) return;
    await onSend(searchText);
    setSearchText("");
  };

  return (
    <form onSubmit={handleSubmit} className="p-3 border-t border-slate-800 bg-slate-950/90 flex items-center gap-2">
      <input
        className="flex-1 rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/70 focus:border-indigo-500/70"
        placeholder="Ask a question about the selected database or table..."
        value={searchText}
        onChange={(e) => setSearchText(e.target.value)}
        disabled={inProgress}
      />
      {!hideStopButton && inProgress && onStop && (
        <button
          type="button"
          onClick={onStop}
      ...
      <button
        type="submit"
        disabled={!searchText.trim() || inProgress}
        className="rounded-lg bg-indigo-500 hover:bg-indigo-500/90 px-3 py-2 text-xs font-medium text-white disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Send
      </button>
    </form>
