...
export function DefaultToolRender({status, name = "", args, result}: ToolCallProps) {
  ...
  const isCountQuery =
    ...
    (args as any).query.toLowerCase().includes("count(");
  ...
  const handleDownloadCsv = () => {
    ...
    const csv = [
      "value",
      ...lines.map((line) => escapeCell(line)),
    ].join("\n");
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {isCountQuery && (
                  <button
                    ...
                    className="ml-2 inline-flex items-center rounded-md border border-gray-300 bg-white px-2 py-1 text-[10px] font-medium text-gray-800 hover:bg-gray-50"
                  >
                    Download Excel
                  </button>
                )}
